...
llm = init_chat_model(f"azure_openai:{deployment_name}", temperature=0.0)
...
def update_memory(store, namespace, messages):
    ...
    user_preferences = store.get(namespace, "user_preferences")
    # Update the memory
    llm = init_chat_model(f"azure_openai:{deployment_name}", temperature=0.0).with_structured_output(UserPreferences)
    result = llm.invoke(
        [
